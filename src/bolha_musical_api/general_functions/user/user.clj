...
(defn criar-usuario-callback
  "Recebo os dados do usuário e o insiro na base, no fim retorno o token"
  [dados]
  (if-let [insert-result (= 1 (query/insert-user-spotify query/db dados))]
    (let [usuario-criado (get-user-by-email (:email dados))]
      (ct/criar-token-user usuario-criado))
    false))

(defn atualizar-usuario-callback
  "Recebo os dados do usuário e o insiro na base, no fim retorno o token"
  [dados]
  (if-let [update-result (= 1 (query/update-user-spotify query/db dados))]
